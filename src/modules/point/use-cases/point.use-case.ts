...
export class PointUseCase {
  ...
  async addPointInput(input: AddPointMemberRequest): Promise<Point> {
    const member = await this.memberService.findOneMemberByAttribute({ where: { memberCode: input.memberCode } })
  ...
  addPointOutput(member: Member, memberLoyalty: MemberLoyalty): AddPointMemberResponse {
    return {
